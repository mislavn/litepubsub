...
impl Subscriber {
    async fn connect(
        ...
        let publisher = tcp::connect(publisher_addr, Bincode::default).await?;
...
impl Publisher {
    async fn start(self) -> io::Result<PublisherAddrs> {
        let mut connecting_publishers = tcp::listen("localhost:0", Bincode::default).await?;
    ...
    async fn start_subscription_manager(mut self) -> io::Result<SocketAddr> {
        let mut connecting_subscribers = tcp::listen("localhost:0", Bincode::default)
            .await?
...
async fn main() -> anyhow::Result<()> {
    ...
    let publisher = publisher::PublisherClient::new(
        ...
        tcp::connect(addrs.publisher, Bincode::default).await?,
    )
    .spawn();
